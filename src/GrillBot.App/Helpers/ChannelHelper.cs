...
public class ChannelHelper
{
    ...
    /// </summary>
    public async Task<IGuild?> GetGuildFromChannelAsync(IChannel? channel, ulong channelId, CancellationToken cancellationToken = default)
    {
        switch (channel)
        {
            case IDMChannel:
        ...
        var channelEntity = await repository.Channel.FindChannelByIdAsync(channelId, null, true, includeDeleted: true, cancellationToken: cancellationToken);
        if (channelEntity == null)
            return null;

        var guildId = channelEntity.GuildId.ToUlong();
        return await DiscordClient.GetGuildAsync(guildId, options: new() { CancelToken = cancellationToken });
    }
}
