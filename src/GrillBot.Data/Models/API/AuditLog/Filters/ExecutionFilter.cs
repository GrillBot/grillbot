...
public class ExecutionFilter : IExtendedFilter, IDictionaryObject
{
    ...
    private bool IsValidCommand(CommandExecution data)
    {
        if (!string.IsNullOrEmpty(Name) && !data.Command.Contains(Name)) return false;
        if (WasSuccess != null && data.IsSuccess != WasSuccess.Value) return false;

        return IsDurationValid(data.Duration);
    }

    private bool IsValidInteraction(InteractionCommandExecuted data)
    {
        if (!string.IsNullOrEmpty(Name) && !data.FullName.Contains(Name)) return false;
        if (WasSuccess != null && data.IsSuccess != WasSuccess.Value) return false;

        return IsDurationValid(data.Duration);
    }

    private bool IsValidJob(JobExecutionData data)
    {
        if (!string.IsNullOrEmpty(Name) && !data.JobName.Contains(Name)) return false;
        if (WasSuccess != null && (!data.WasError) != WasSuccess.Value) return false;
