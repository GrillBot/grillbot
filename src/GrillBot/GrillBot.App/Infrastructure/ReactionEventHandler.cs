...
public abstract class ReactionEventHandler
{
    ...
    protected bool TryGetEmbedAndMetadata<TMetadata>(IUserMessage message, IEmote reaction, out IEmbed embed, out TMetadata metadata) where TMetadata : IEmbedMetadata, new()
    {
        ...
        return message.ReferencedMessage != null && embed.TryParseMetadata(out metadata);
    }

    protected static int GetNextPageNumber(int current, int maxPages, IEmote emote)
    {
        var newPage = current;
