...
public class RolesReaderTests : CommandActionTest<RolesReader>
{
    ...
    {
        new RoleBuilder().SetColor(Color.Blue).SetIdentity(Consts.RoleId, Consts.RoleName).SetPermissions(GuildPermissions.All).SetPosition(1)
            .SetTags(RoleHelper.CreateTags(Consts.UserId, null, false)).Build(),
        new RoleBuilder().SetColor(Color.Red).SetIdentity(Consts.RoleId + 1, Consts.RoleName).SetPermissions(GuildPermissions.All).SetPosition(2).SetTags(RoleHelper.CreateTags(null, null, true))
            .Build(),
        new RoleBuilder().SetColor(Color.Green).SetIdentity(Consts.RoleId + 2, Consts.RoleName).SetPermissions(GuildPermissions.Webhook).SetPosition(2)
            .SetTags(RoleHelper.CreateTags(null, null, true)).Build()
    };

    private static readonly IGuild EmptyGuild = new GuildBuilder().SetIdentity(Consts.GuildId, Consts.GuildName).SetRoles(Roles).Build();

    private static readonly IGuildUser[] GuildUsers =
    {
        new GuildUserBuilder().SetIdentity(Consts.UserId, Consts.Username, Consts.Discriminator).AsBot().SetGuild(EmptyGuild).SetRoles(new[] { Roles[0] }).Build(),
        new GuildUserBuilder().SetIdentity(Consts.UserId + 1, Consts.Username, Consts.Discriminator).AsBot().SetGuild(EmptyGuild).SetRoles(new[] { Roles[1] }).Build()
    };

    protected override IGuild Guild => new GuildBuilder().SetIdentity(EmptyGuild.Id, EmptyGuild.Name).SetRoles(Roles).SetGetUsersAction(GuildUsers).Build();
